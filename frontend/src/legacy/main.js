...
const routeMappings = {
  'login.html': '/login',
  'master.html': '/master',
  'master-create.html': '/master/create',
  'master-consult.html': '/master/consult',
  'master-list.html': '/master/list',
  'master-sales.html': '/master/sales',
  'master-sku-points.html': '/master/sku-points',
  'master-scripts.html': '/master/scripts',
  'influencer.html': '/influencer',
  'influencer-plan.html': '/influencer/plan',
  'influencer-performance.html': '/influencer/performance',
  'pinklovers-master.html': '/pinklovers/master',
  'pinklovers-influencer.html': '/pinklovers/influencer',
  'script-view.html': '/scripts/view',
  'aceite-termos.html': '/aceite-termos'
};

const resolveAppRoute = (target = '') => {
  if (!target) return '/';
  if (/^https?:/i.test(target)) return target;
  if (target.startsWith('/')) return target;
  const [rawPath, search = ''] = target.split('?');
  const mapped = routeMappings[rawPath] || (rawPath.endsWith('.html') ? `/${rawPath.replace(/\.html$/i, '')}` : rawPath);
  return search ? `${mapped}?${search}` : mapped;
};

const redirectTo = (page) => {
  const destination = resolveAppRoute(page);
...
const redirectToInfluencerEdit = (id) => {
  redirectTo(`master-create.html?id=${id}`);
};
...
export {
  ...
  initLoginPage,
  initMasterHomePage,
  initMasterCreatePage,
  initMasterConsultPage,
  initMasterListPage,
  initMasterSalesPage,
  initMasterScriptsPage,
  initMasterSkuPointsPage,
  initInfluencerPage,
  initTermAcceptancePage
};
