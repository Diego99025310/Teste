(() => {
  ...
  const initMasterCreatePage = () => {
    ...
    const updateWhatsappMessage = (payload = {}) => {
      ...
      resetCopyWhatsappButton(Boolean(message));
    };
    ...
    const handlePasswordRegeneration = () => {
      ...
      if (form?.dataset?.mode === 'edit') {
        setMessage(
          messageEl,
          'Nova senha provisória gerada. Salve as alterações para aplicá-la.',
          'info'
        );
      }
    };
    ...
    const resetForm = ({ clearMessage = false, preserveSummary = false } = {}) => {
      ...
      if (passwordInput) {
        passwordInput.placeholder = 'Senha provisória numérica gerada automaticamente';
    ...
    const loadInfluencerForEdit = async (id) => {
      ...
      try {
        ...
        if (passwordInput) {
          passwordInput.placeholder = 'Clique em "Gerar nova senha provisória" para redefinir a senha.';
